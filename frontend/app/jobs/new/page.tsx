...
const PRESETS = [
  {
    name: 'Balanced',
    icon: '⚖️',
    desc: 'General hiring — all factors matter equally',
  ...
  },
  {
    ...
    icon: '💻',
    desc: 'Engineering, data science, DevOps roles',
  ...
  },
  {
    name: 'Leadership',
    icon: '🎯',
    desc: 'Managers, directors, team leads',
  ...
  },
  {
    name: 'Sales & BD',
    icon: '📈',
    desc: 'Sales, business development, account management',
  ...
  },
  {
    name: 'Senior / Expert',
    icon: '🏆',
    desc: 'Senior ICs, architects, principals — high bar',
  ...
  },
  {
    name: 'Fresh Graduate',
    icon: '🎓',
    desc: 'Entry level, campus hiring, trainee roles',
...
export default function NewJobPage() {
  ...
  const updateImportance = (skill: string, importance: string) => {
    setForm(f => ({
      ...f,
      required_skills: f.required_skills.map(s => s.skill === skill ? { ...s, importance } : s),
    ...
    try {
      const job = await api.createJob({
        ...
        scoring_weights: {
          ...
          shortlist_threshold: form.shortlist_threshold,
          review_threshold: form.review_threshold,
        }
      })
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-6 py-5 space-y-4">
            <h2 className="font-semibold text-[#1B4F8A] flex items-center gap-2">
              📋 Basic Information
            </h2>
            <div>
              ...
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-[#1B4F8A] focus:ring-2 focus:ring-blue-100"
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
                Full JD Text (optional — helps AI score better)
              </label>
                  ...
                  <button key={preset.name} onClick={() => applyPreset(preset)}
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`text-sm font-semibold ${
                        activePreset === preset.name ? 'text-[#1B4F8A]' : 'text-gray-800'
                      }`}>
                        {preset.name}
                      </span>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-green-50 rounded-xl border border-green-200 p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-lg font-bold text-green-700 w-12 text-right">
                      {form.shortlist_threshold}%
                    </span>
                  </div>
                  <p className="text-xs text-green-600 mt-1">
                    Candidates scoring {form.shortlist_threshold}%+ are shortlisted
                  </p>
                </div>
                <div className="bg-yellow-50 rounded-xl border border-yellow-200 p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <span className="text-lg font-bold text-yellow-700 w-12 text-right">
                      {form.review_threshold}%
                    </span>
                  </div>
                  <p className="text-xs text-yellow-600 mt-1">
                    Below {form.review_threshold}% is automatically passed
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-6 py-5 space-y-3">
            ...
            <textarea value={form.custom_instructions}
              ...
              rows={3}
              className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-[#1B4F8A]"
              placeholder='e.g. "Prefer product company backgrounds. Value open source contributions. Penalize more than 3 jobs in 4 years."' />
          </div>
          ...
          <div className="flex justify-end gap-3 pb-8">
            ...
            <button onClick={handleSave} disabled={saving}
              className="px-8 py-2.5 bg-[#1B4F8A] text-white rounded-lg text-sm font-semibold hover:bg-[#133A66] disabled:opacity-50 transition shadow-sm">
